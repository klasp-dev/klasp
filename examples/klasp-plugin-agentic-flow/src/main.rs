//! `klasp-plugin-agentic-flow` — reference plugin for the klasp v0 plugin protocol.
//!
//! This plugin audits the receipts written by the agentic-flow orchestrator
//! (`~/.claude/agentic-flow/`) and reports whether the required workflow steps
//! were completed (and stayed fresh) for the current diff. It is a **read-only
//! auditor**: it never runs the workflow, never infers state from chat, and
//! never creates or mutates receipts.
//!
//! This crate is also a canonical "fork me" starting point for third-party
//! klasp plugin authors. It implements `--describe` and `--gate` exactly as the
...
//! 3. `src/runner.rs` — replace the audit logic with your own check.
...
const PLUGIN_NAME: &str = "klasp-plugin-agentic-flow";

/// Config `type` values this plugin supports (informational at v0).
const CONFIG_TYPES: &[&str] = &["agentic-flow"];

#[derive(Parser, Debug)]
#[command(
    name = PLUGIN_NAME,
    about = "Reference klasp plugin that audits agentic-flow receipts (v0 protocol)",
    long_about = None,
)]
struct Args {
    ...
    /// Read PluginGateInput from stdin, audit receipts, print PluginGateOutput.
...
/// Read `PluginGateInput` from stdin, audit receipts, emit `PluginGateOutput`.
...
fn cmd_gate() {
    ...
    let json = serde_json::to_string(&output)
        .unwrap_or_else(|_| String::from(
            r#"{"protocol_version":0,"verdict":"warn","findings":[{"severity":"warn","rule":"klasp-plugin-agentic-flow/output-serialize-failed","message":"internal error: PluginGateOutput serialize failed"}]}"#,
        ));
    write_stdout_line(&json);
}
