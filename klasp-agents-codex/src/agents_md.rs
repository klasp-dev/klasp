...
use klasp_core::text::managed_block::{self, BlockError, Markers};
use thiserror::Error;
...
impl From<BlockError> for AgentsMdError {
    fn from(err: BlockError) -> Self {
        match err {
            BlockError::MalformedMarkers => AgentsMdError::MalformedMarkers,
        }
    }
}
...
pub fn uninstall_block(existing: &str) -> Result<String, AgentsMdError> {
    Ok(managed_block::uninstall_block(existing, &MARKERS, None)?)
}
