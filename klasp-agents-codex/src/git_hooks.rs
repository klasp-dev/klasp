...
pub fn detect_conflict(existing: &str) -> Option<HookConflict> {
    // husky: shim filename match. Both v8+ (`_/husky.sh`) and the
    // current v9 (`_/h`) variants reference `husky` somewhere, but the
    // canonical fingerprint is the dotted-source line.
    if existing.contains(".husky/")
        || existing.contains("_/husky.sh")
        || existing.contains("husky.sh\"")
    {
