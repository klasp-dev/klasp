...
//!
//! Codex has no programmatic gate equivalent (see
//! [docs/design.md §1](../../docs/design.md)), so its enforcement story
//! relies on real `.git/hooks/pre-commit` and `.git/hooks/pre-push`
//! scripts. This module is the analogue of [`crate::agents_md`] for those
//! shell files: it owns the literal `klasp managed start … klasp managed
//! end` block, knows how to splice it into an existing user hook without
//! disturbing sibling content, and recognises the on-disk fingerprints of
//! the popular hook managers (husky, lefthook, pre-commit framework) so
//! `install` can decline to clobber them.
//!
//! ## Trigger taxonomy
//!
//! There are two hook files per repo, identical except for the `--trigger`
//! arg klasp passes through to `klasp gate`. [`HookKind::Commit`] →
//! `pre-commit`, [`HookKind::Push`] → `pre-push`. Both export
//! `KLASP_GATE_SCHEMA` so [`klasp_core::GateProtocol::check_schema_env`]
//! can detect drift between the shim and the binary.
//!
//! ## Idempotency contract
//!
//! Same shape as [`crate::agents_md`]:
//!
//! - `install_block(install_block(input))` == `install_block(input)`
//! - `uninstall_block(install_block(input))` == `input`, modulo a
//!   tolerated trailing-newline normalisation that matches the POSIX
//!   `<content>\n` convention. See the docstring on [`uninstall_block`]
//!   for the exact tolerated case.
//! - Bytes outside the managed block are returned unchanged.
//!
//! ## Conflict detection
//!
//! [`detect_conflict`] sniffs the existing file content for fingerprints
//! of three popular hook managers. When one matches, callers should
//! *skip* writing the hook entirely and surface a [`HookConflict`] in
//! their install report — never fail the install, never overwrite the
//! foreign tool's content. The fingerprints are the public, version-stable
//! markers each tool itself drops in (or `source`s in) the hook, so
//! detection is robust to per-version cosmetic drift.
...
pub fn detect_conflict(existing: &str) -> Option<HookConflict> {
    // husky: match the dotted-source line husky writes into the hook.
    // v8 sources `…/_/husky.sh`; v9 shortened the shim to `…/_/h`. We
    // anchor on the closing quote so a user comment that merely
    // mentions `_/h` or `husky.sh` somewhere doesn't false-positive —
    // the substrings below only appear at the end of the actual
    // `. "$(dirname -- "$0")/_/<shim>"` line husky generates.
    if existing.contains("_/husky.sh\"") || existing.contains("_/h\"") {
        return Some(HookConflict::Husky);
    }
    // lefthook: explicit anti-deletion sentinel. We *also* require a
    // `lefthook` invocation token to avoid matching hooks that merely
    // mention the marker in a comment.
    if existing.contains("DON'T REMOVE THIS LINE (lefthook)") && existing.contains("lefthook") {
        return Some(HookConflict::Lefthook);
    }
    // pre-commit framework: stable banner emitted by every generated hook.
