...
> **Status (2026-06-10).** v0.1 shipped on `main` at [`234908e`](https://github.com/klasp-dev/klasp/commit/234908e) (PR [#17](https://github.com/klasp-dev/klasp/pull/17), W6-7). Since then v0.2 (Codex + named recipes), v0.2.5 (parallel execution + monorepo discovery), v0.3 (Aider + the experimental plugin model), v0.4.0 (`klasp init --adopt`, `klasp setup`), and v0.5.0 (cross-surface hook-conflict detection, the `klasp-plugin-agentic-flow` receipts gate, and `klasp demo`) have all shipped to `main`. The agent-surface conformance matrix is now a tracked contract ([#68](https://github.com/klasp-dev/klasp/issues/68)). The per-milestone checklists below predate these releases and are kept as a historical record of the original commitments.
