...
pub fn write_klasp_toml(
    ...
    atomic_write_text(&target, &toml_content)?;
    Ok(target)
}
...
fn generate_toml(plan: &AdoptionPlan) -> String {
    ...
    let mut out = format!(
        "# klasp.toml — generated by `klasp init --adopt --mode mirror`\n\
    ...
    for finding in &plan.findings {
        for check in &finding.proposed_checks {
            ...
            let timeout = check.timeout_secs.unwrap_or(120);
...
/// Atomic write via tempfile + rename into `path`'s parent directory.
